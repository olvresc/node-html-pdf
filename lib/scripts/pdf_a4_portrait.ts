...
function getContent(page) {
  return page.evaluate(function () {
    function getElements(doc, wildcard): any {
...
function createSection(section, content, options) {
  ...
  return {
    ...
    contents: phantom.callback(function (pageNum, numPages) {
      ...
      return (
        (html || o.default || c.default || '').replace(/{{page}}/g, pageNumFinal).replace(/{{pages}}/g, numPagesFinal) +
        content.styles
      );
    }),
  };
...
function definePaperOrientation(options) {
  var paper: any = { border: options.border || '0' };
